 ...
 */
public class BinaryNodeParameterProviderFactoryTest extends AbstractRepositoryTestCase {
    ...
    @Test
    public void testPathInfoSplitting() throws Exception {
        ...
        String imageNodeUuid = imageNode.getIdentifier();
        when(req.getPathInfo()).thenReturn("/generator/test/"+ imageNodeUuid + "/MaxMustermann.png");

        // WHEN
        final BinaryNodeParameterProviderFactory factory = new BinaryNodeParameterProviderFactory();
        ParameterProvider<Node> params = factory.newParameterProviderFor(req);

        // THEN
        assertMatches("Returned node must be same.", params.getParameter().getIdentifier(), imageNode.getIdentifier());
    }
}
